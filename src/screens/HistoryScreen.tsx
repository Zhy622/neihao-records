...
export function HistoryScreen() {
  ...
  return (
    <Screen>
      ...
      <HapticPressable style={({ pressed }) => [styles.filterButton, pressed && styles.pressed]} onPress={() => filterSheetRef.current?.present()}>
        <Ionicons name="options-outline" size={18} color={colors.primary} />
        <Text style={styles.filterButtonText}>{activeFilters ? `筛选条件 · ${activeFilters}` : '筛选条件'}</Text>
      </HapticPressable>
      <View style={styles.list}>
        ...
        {!records.length ? <EmptyState icon="search-outline" title="没有找到记录" description="换一个时间、分类或关键词再看看。" /> : null}
      </View>
      <BottomSheetModal
        ref={filterSheetRef}
...
const styles = StyleSheet.create({
  ...
  search: { color: colors.text, backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 20, paddingHorizontal: 15, paddingVertical: 13, fontFamily: fonts.regular },
  filterButton: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 18, backgroundColor: colors.primarySoft },
